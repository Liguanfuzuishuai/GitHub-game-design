...
if reco_type == 0:
    ...
    for img_path in images_names_for_test:
        ...
        if test_img is not None:
            ...
            annotated_img = cv2.cvtColor(resized_img, cv2.COLOR_GRAY2BGR)
            font = cv2.FONT_HERSHEY_SIMPLEX
            font_scale = 0.45
            font_color = (0, 255, 0)
            thickness = 1
            cv2.putText(annotated_img, find_name, (5, 20), font, font_scale, font_color, thickness)
            annotated_img = cv2.resize(annotated_img, (annotated_img.shape[1]*10, annotated_img.shape[0]*10))
            cv2.imshow(f"Test Image {i}", annotated_img)
            cv2.waitKey(0)
            cv2.destroyWindow(f"Test Image {i}")
...
if reco_type == 2:
    ...
    # 如果要检测不同的group图片改这里的路径
    frame = cv2.imread(dir+ "group_image.png")
    ...
    for(x, y, w, h) in faces:
        ...
        font_color = (255, 0, 0)
        font_stroke = 5
        cv2.putText(frame, name + str(i), (x, y), font, 2, font_color, font_stroke, cv2.LINE_AA)
        i += 1
